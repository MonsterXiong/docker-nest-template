...
@Injectable()
export class DpMetaEntityService extends CommonService {
  constructor(
    @InjectRepository(DpMetaEntity)
    protected readonly repository: Repository<DpMetaEntity>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpMetaEntityTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpMetaEntity && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpMetaEntity[];
  ...
  async save(entity: Partial<DpMetaEntity>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpMetaEntity>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpMetaEntity>,req) {
  ...
  async updateBatch(entity: Partial<DpMetaEntity>[],req) {
  ...
  _createEntity(entity: Partial<DpMetaEntity>,req) {
    const dpMetaEntity = new DpMetaEntity(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpMetaEntity, entity); // 复制属性
    return dpMetaEntity;
  }
  _updateEntity(entity: Partial<DpMetaEntity>,req) {
    const dpMetaEntity = new DpMetaEntity(); // 创建实体实例
