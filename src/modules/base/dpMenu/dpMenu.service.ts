...
export class DpMenuService extends CommonService {
  ...
  saveBatch(entity: DpMenu[]) {
    return this.repository.save(entity);
  }

  async update(entity: DpMenu) {
    const existingData = await this.findOne(entity. id);
    const mergedDpMenu = this.repository.merge(existingData, entity);
    this.repository.update(entity. id, mergedDpMenu)
  }

  async updateBatch(entity: DpMenu[]) {
