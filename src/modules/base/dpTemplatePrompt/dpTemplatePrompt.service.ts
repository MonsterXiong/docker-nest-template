...
@Injectable()
export class DpTemplatePromptService extends CommonService {
  constructor(
    @InjectRepository(DpTemplatePrompt)
    protected readonly repository: Repository<DpTemplatePrompt>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpTemplatePromptTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpTemplatePrompt && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpTemplatePrompt[];
  ...
  async save(entity: Partial<DpTemplatePrompt>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpTemplatePrompt>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpTemplatePrompt>,req) {
  ...
  async updateBatch(entity: Partial<DpTemplatePrompt>[],req) {
  ...
  _createEntity(entity: Partial<DpTemplatePrompt>,req) {
    const dpTemplatePrompt = new DpTemplatePrompt(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpTemplatePrompt, entity); // 复制属性
    return dpTemplatePrompt;
  }
  _updateEntity(entity: Partial<DpTemplatePrompt>,req) {
    const dpTemplatePrompt = new DpTemplatePrompt(); // 创建实体实例
