...
@Injectable()
export class DpNodeService extends CommonService {
  constructor(
    @InjectRepository(DpNode)
    protected readonly repository: Repository<DpNode>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpNodeTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpNode && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpNode[];
  ...
  async save(entity: Partial<DpNode>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpNode>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpNode>,req) {
  ...
  async updateBatch(entity: Partial<DpNode>[],req) {
  ...
  _createEntity(entity: Partial<DpNode>,req) {
    const dpNode = new DpNode(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpNode, entity); // 复制属性
    return dpNode;
  }
  _updateEntity(entity: Partial<DpNode>,req) {
    const dpNode = new DpNode(); // 创建实体实例
