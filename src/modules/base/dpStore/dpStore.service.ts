...
@Injectable()
export class DpStoreService extends CommonService {
  constructor(
    @InjectRepository(DpStore)
    protected readonly repository: Repository<DpStore>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpStoreTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpStore && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpStore[];
  ...
  async save(entity: Partial<DpStore>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpStore>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpStore>,req) {
  ...
  async updateBatch(entity: Partial<DpStore>[],req) {
  ...
  _createEntity(entity: Partial<DpStore>,req) {
    const dpStore = new DpStore(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpStore, entity); // 复制属性
    return dpStore;
  }
  _updateEntity(entity: Partial<DpStore>,req) {
    const dpStore = new DpStore(); // 创建实体实例
