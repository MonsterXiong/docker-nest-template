...
@Injectable()
export class DpMetaEnumService extends CommonService {
  constructor(
    @InjectRepository(DpMetaEnum)
    protected readonly repository: Repository<DpMetaEnum>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpMetaEnumTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpMetaEnum && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpMetaEnum[];
  ...
  async save(entity: Partial<DpMetaEnum>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpMetaEnum>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpMetaEnum>,req) {
  ...
  async updateBatch(entity: Partial<DpMetaEnum>[],req) {
  ...
  _createEntity(entity: Partial<DpMetaEnum>,req) {
    const dpMetaEnum = new DpMetaEnum(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpMetaEnum, entity); // 复制属性
    return dpMetaEnum;
  }
  _updateEntity(entity: Partial<DpMetaEnum>,req) {
    const dpMetaEnum = new DpMetaEnum(); // 创建实体实例
