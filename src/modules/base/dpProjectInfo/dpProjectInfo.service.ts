...
export class DpProjectInfoService extends CommonService {
  ...
  downloadDpProjectInfoTemplate() {
    ...
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpProjectInfo
    );
  ...
  save(entity: DpProjectInfo) {
    return this.repository.save(entity);

  }
  saveBatch(entity: DpProjectInfo[]) {
    return this.repository.save(entity);
  }

  async update(entity: DpProjectInfo) {
    const existingData = await this.findOne(entity. id);
    const mergedDpProjectInfo = this.repository.merge(existingData, entity);
    this.repository.update(entity. id, mergedDpProjectInfo)
  }

  async updateBatch(entity: DpProjectInfo[]) {
