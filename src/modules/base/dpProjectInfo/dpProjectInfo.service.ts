...
@Injectable()
export class DpProjectInfoService extends CommonService {
  constructor(
    @InjectRepository(DpProjectInfo)
    protected readonly repository: Repository<DpProjectInfo>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpProjectInfoTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpProjectInfo && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpProjectInfo[];
  ...
  async save(entity: Partial<DpProjectInfo>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpProjectInfo>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpProjectInfo>,req) {
  ...
  async updateBatch(entity: Partial<DpProjectInfo>[],req) {
  ...
  _createEntity(entity: Partial<DpProjectInfo>,req) {
    const dpProjectInfo = new DpProjectInfo(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpProjectInfo, entity); // 复制属性
    return dpProjectInfo;
  }
  _updateEntity(entity: Partial<DpProjectInfo>,req) {
    const dpProjectInfo = new DpProjectInfo(); // 创建实体实例
