...
export class DpProjectService extends CommonService {
  ...
  downloadDpProjectTemplate() {
    ...
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpProject
    );
  ...
  save(entity: DpProject) {
    return this.repository.save(entity);

  }
  saveBatch(entity: DpProject[]) {
    return this.repository.save(entity);
  }

  async update(entity: DpProject) {
    const existingData = await this.findOne(entity. id);
    const mergedDpProject = this.repository.merge(existingData, entity);
    this.repository.update(entity. id, mergedDpProject)
  }

  async updateBatch(entity: DpProject[]) {
