...
@Injectable()
export class DpProjectService extends CommonService {
  constructor(
    @InjectRepository(DpProject)
    protected readonly repository: Repository<DpProject>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpProjectTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpProject && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpProject[];
  ...
  async saveBatch(newObjectList: Partial<DpProject>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpProject>,req) {
  ...
  async updateBatch(entity: Partial<DpProject>[],req) {
  ...
  _createEntity(entity: Partial<DpProject>,req) {
    const dpProject = new DpProject(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpProject, entity); // 复制属性
    return dpProject;
  }
  _updateEntity(entity: Partial<DpProject>,req) {
    const dpProject = new DpProject(); // 创建实体实例
