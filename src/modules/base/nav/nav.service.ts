...
export class NavService extends CommonService {
  ...
  saveBatch(entity: Nav[]) {
    return this.repository.save(entity);
  }

  async update(entity: Nav) {
    const existingData = await this.findOne(entity. id);
    const mergedNav = this.repository.merge(existingData, entity);
    this.repository.update(entity. id, mergedNav)
  }

  async updateBatch(entity: Nav[]) {
