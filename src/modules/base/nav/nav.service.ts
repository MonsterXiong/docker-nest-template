...
@Injectable()
export class NavService extends CommonService {
  constructor(
    @InjectRepository(Nav)
    protected readonly repository: Repository<Nav>,
  ) { 
    super(repository);
  }
  ...
  downloadNavTemplate() {
    ...
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === Nav && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as Nav[];
  ...
  async saveBatch(newObjectList: Partial<Nav>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<Nav>,req) {
  ...
  async updateBatch(entity: Partial<Nav>[],req) {
  ...
  _createEntity(entity: Partial<Nav>,req) {
    const nav = new Nav(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(nav, entity); // 复制属性
    return nav;
  }
  _updateEntity(entity: Partial<Nav>,req) {
    const nav = new Nav(); // 创建实体实例
