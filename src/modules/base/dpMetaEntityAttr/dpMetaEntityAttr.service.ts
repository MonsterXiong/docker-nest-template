...
@Injectable()
export class DpMetaEntityAttrService extends CommonService {
  constructor(
    @InjectRepository(DpMetaEntityAttr)
    protected readonly repository: Repository<DpMetaEntityAttr>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpMetaEntityAttrTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpMetaEntityAttr && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpMetaEntityAttr[];
  ...
  async save(entity: Partial<DpMetaEntityAttr>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpMetaEntityAttr>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpMetaEntityAttr>,req) {
  ...
  async updateBatch(entity: Partial<DpMetaEntityAttr>[],req) {
  ...
  _createEntity(entity: Partial<DpMetaEntityAttr>,req) {
    const dpMetaEntityAttr = new DpMetaEntityAttr(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpMetaEntityAttr, entity); // 复制属性
    return dpMetaEntityAttr;
  }
  _updateEntity(entity: Partial<DpMetaEntityAttr>,req) {
    const dpMetaEntityAttr = new DpMetaEntityAttr(); // 创建实体实例
