...
@Injectable()
export class NavToolService extends CommonService {
  constructor(
    @InjectRepository(NavTool)
    protected readonly repository: Repository<NavTool>,
  ) { 
    super(repository);
  }
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as NavTool[];
  ...
  saveBatch(entity: NavTool[]) {
    return this.repository.save(entity);
  }

  async update(entity: NavTool) {
    const existingData = await this.findOne(entity. id);
    const mergedNavTool = this.repository.merge(existingData, entity);
    this.repository.update(entity. id, mergedNavTool)
  }

  async updateBatch(entity: NavTool[]) {
