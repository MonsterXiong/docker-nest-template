...
export class DocService extends CommonService {
  constructor(
    ...
    protected readonly repository: Repository<Doc>,
  ) { 
    super(repository);
  }
  ...
  downloadDocTemplate() {
    ...
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === Doc && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as Doc[];
  ...
  async save(entity: Partial<Doc>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<Doc>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<Doc>,req) {
  ...
  async updateBatch(entity: Partial<Doc>[],req) {
  ...
  _createEntity(entity: Partial<Doc>,req) {
    const doc = new Doc(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(doc, entity); // 复制属性
    return doc;
  }
  _updateEntity(entity: Partial<Doc>,req) {
    const doc = new Doc(); // 创建实体实例
