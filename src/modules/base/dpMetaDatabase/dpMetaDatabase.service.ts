...
@Injectable()
export class DpMetaDatabaseService extends CommonService {
  constructor(
    @InjectRepository(DpMetaDatabase)
    protected readonly repository: Repository<DpMetaDatabase>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpMetaDatabaseTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpMetaDatabase && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpMetaDatabase[];
  ...
  async save(entity: Partial<DpMetaDatabase>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpMetaDatabase>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpMetaDatabase>,req) {
  ...
  async updateBatch(entity: Partial<DpMetaDatabase>[],req) {
  ...
  _createEntity(entity: Partial<DpMetaDatabase>,req) {
    const dpMetaDatabase = new DpMetaDatabase(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpMetaDatabase, entity); // 复制属性
    return dpMetaDatabase;
  }
  _updateEntity(entity: Partial<DpMetaDatabase>,req) {
    const dpMetaDatabase = new DpMetaDatabase(); // 创建实体实例
