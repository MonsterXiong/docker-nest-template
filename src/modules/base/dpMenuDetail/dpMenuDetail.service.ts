...
export class DpMenuDetailService extends CommonService {
  ...
  downloadDpMenuDetailTemplate() {
    ...
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpMenuDetail
    );
  ...
  save(entity: DpMenuDetail) {
    return this.repository.save(entity);

  }
  saveBatch(entity: DpMenuDetail[]) {
    return this.repository.save(entity);
  }

  async update(entity: DpMenuDetail) {
    const existingData = await this.findOne(entity. id);
    const mergedDpMenuDetail = this.repository.merge(existingData, entity);
    this.repository.update(entity. id, mergedDpMenuDetail)
  }

  async updateBatch(entity: DpMenuDetail[]) {
