...
@Injectable()
export class DpMenuDetailService extends CommonService {
  constructor(
    @InjectRepository(DpMenuDetail)
    protected readonly repository: Repository<DpMenuDetail>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpMenuDetailTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpMenuDetail && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpMenuDetail[];
  ...
  async save(entity: Partial<DpMenuDetail>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpMenuDetail>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpMenuDetail>,req) {
  ...
  async updateBatch(entity: Partial<DpMenuDetail>[],req) {
    for await (const entityItem of entity) {
      this.update(this._updateEntity(entityItem,req),req)
    }
  }

  _createEntity(entity: Partial<DpMenuDetail>,req) {
    const dpMenuDetail = new DpMenuDetail(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpMenuDetail, entity); // 复制属性
    return dpMenuDetail;
  }
  _updateEntity(entity: Partial<DpMenuDetail>,req) {
    const dpMenuDetail = new DpMenuDetail(); // 创建实体实例
