...
@Injectable()
export class DpEnvConfigService extends CommonService {
  constructor(
    @InjectRepository(DpEnvConfig)
    protected readonly repository: Repository<DpEnvConfig>,
  ) { 
    super(repository);
  }
  ...
  // 获取导入模板
  downloadDpEnvConfigTemplate() {
    // 获取实体的元数据
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpEnvConfig && !column.propertyName.startsWith('sys')
    );
  ...
  async importFromExcel(file: Buffer, req: Request) {
    ...
    const data = xlsx.utils.sheet_to_json(ws) as DpEnvConfig[];
  ...
  async save(entity: Partial<DpEnvConfig>,req) {
    if(entity.id){
  ...
  async saveBatch(newObjectList: Partial<DpEnvConfig>[],oldKeyList:string[],req) {
  ...
  async update(entity: Partial<DpEnvConfig>,req) {
  ...
  async updateBatch(entity: Partial<DpEnvConfig>[],req) {
    for await (const entityItem of entity) {
      this.update(this._updateEntity(entityItem,req),req)
    }
  }

  _createEntity(entity: Partial<DpEnvConfig>,req) {
    const dpEnvConfig = new DpEnvConfig(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(dpEnvConfig, entity); // 复制属性
    return dpEnvConfig;
  }
  _updateEntity(entity: Partial<DpEnvConfig>,req) {
    const dpEnvConfig = new DpEnvConfig(); // 创建实体实例
