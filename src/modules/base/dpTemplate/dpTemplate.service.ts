...
export class DpTemplateService extends CommonService {
  ...
  downloadDpTemplateTemplate() {
    ...
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === DpTemplate
    );
  ...
  save(entity: DpTemplate) {
    return this.repository.save(entity);

  }
  saveBatch(entity: DpTemplate[]) {
    return this.repository.save(entity);
  }

  async update(entity: DpTemplate) {
    const existingData = await this.findOne(entity. id);
    const mergedDpTemplate = this.repository.merge(existingData, entity);
    this.repository.update(entity. id, mergedDpTemplate)
  }

  async updateBatch(entity: DpTemplate[]) {
