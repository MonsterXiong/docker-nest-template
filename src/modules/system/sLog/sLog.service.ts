...
export class SLogService extends CommonService {
  ...
  downloadSLogTemplate() {
    ...
    const metadata = getMetadataArgsStorage().columns.filter(
      column => column.target === SLog && !column.propertyName.startsWith('sys')
    );
  ...
  async save(entity: Partial<SLog>,req) {
    if(entity.id){
  ...
  async saveBatch(entity: Partial<SLog>[],req) {
    for await (const entityItem of entity) {
      this.save(entityItem,req)
    }
  }

  async update(entity: Partial<SLog>,req) {
  ...
  async updateBatch(entity: Partial<SLog>[],req) {
    for await (const entityItem of entity) {
      this.update(this._updateEntity(entityItem,req),req)
    }
  }

  _createEntity(entity: Partial<SLog>,req) {
    const sLog = new SLog(); // 创建实体实例
    if(!entity.id){
      entity.id = nanoid()
    }
    this.setCreateInfo(entity,req)
    Object.assign(sLog, entity); // 复制属性
    return sLog;
  }
  _updateEntity(entity: Partial<SLog>,req) {
    const sLog = new SLog(); // 创建实体实例
